...
/**
 * This service needed to properly apply the IPTABLES/IP6TABLES rules upon reboot.  Some devices were having problems
 * applying the rules on boot using the old method.  Judging by data given to me by my testers some devices were
 * simply taking too long to boot up and the old method was more or less being skipped particularly on devices
 * with large amounts of rules.
 * 
 * By making the Boot up use a service it allows longer running
 * background operations which hopefully should fix this issue. 
 * 
...
public class setRulesOnBootService extends IntentService
{
	...
	protected void BootUpComplete()
	{
		...
		if (Api.isEnabled(context.getApplicationContext()))
		{
			...
			{
				Toast.makeText(this,  "Unable to apply the firewall rules", Toast.LENGTH_SHORT).show();
	...
	// called from the previous performAction in order to be able to pass the correct
	// variables around due to the use of Intents
	public static void performAction(Context context, String bootupaction, Bundle extras)
	{
		if ((context == null) || (bootupaction == null) || bootupaction.equals(""))
		return;
